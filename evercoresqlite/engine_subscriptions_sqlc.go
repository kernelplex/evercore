...
func (s *SqliteStorageEngine) GetEventsForSubscription(tx evercore.StorageEngineTxInfo, ctx context.Context, sub *evercore.Subscription, limit int) ([]evercore.SerializedEvent, error) {
    db := s.maybeWrapTx(tx)
    q := New(db)
    params := GetEventsForSubscriptionParams{
    ...
    // hasMulti via a cheap check: if sub.EventTypeID is nil, assume possible multi; engine can still handle empty
    if sub.EventTypeID == nil { params.UseMulti = 1 }
    rows, err := q.GetEventsForSubscription(ctx, params)
